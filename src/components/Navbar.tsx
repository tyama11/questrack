...
export const Navbar: React.FC<NavbarProps> = ({ activeTab, setActiveTab }) => {
  ...
  // テーマ切り替えサイクル: system ➔ light ➔ dark ➔ system
  const cycleTheme = () => {
  ...
  // 言語切り替えサイクル: system ➔ ja ➔ en ➔ system
  const cycleLanguage = () => {
    if (languageMode === 'system') setLanguageMode(currentLanguage === 'ja' ? 'en' : 'ja');
    else if (languageMode === 'en') setLanguageMode('ja');
    else setLanguageMode('system');
  };
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800/70 p-1.5 rounded-xl border border-slate-200/80 dark:border-slate-700/60">
            ...
            >
              ...
              <span>{dict.nav.data}</span>
            </button>
          </nav>

          {/* 右側: 状況インジケーター & テーマ・言語設定 */}
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="hidden lg:flex items-center">
              {totalIncorrectAll > 0 ? (
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900/60 text-rose-700 dark:text-rose-300 text-xs font-medium">
                  ...
                  <span>{dict.nav.reviewNeeded}</span>
                  <span className="font-bold text-rose-600 dark:text-rose-400 text-sm">
                    {totalIncorrectAll}{dict.nav.questionsUnit}
                  </span>
                </div>
              ) : (
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900/60 text-emerald-700 dark:text-emerald-300 text-xs font-medium">
                  ...
                  <span>{dict.nav.allCleared}</span>
                </div>
              )}
            </div>

            {/* テーマ切り替えボタン (OS連動 / Light / Dark) */}
            <button
              onClick={cycleTheme}
              className="p-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/80 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors flex items-center gap-1 text-xs"
              title={
                themeMode === 'system'
            ...
            >
              {themeMode === 'system' && (
                <>
                  ...
                  <span className="hidden xl:inline text-[11px] font-medium text-slate-500 dark:text-slate-400">OS</span>
                </>
              )}
              {themeMode === 'light' && (
                <>
                  <Sun className="w-4 h-4 text-amber-500" />
                  <span className="hidden xl:inline text-[11px] font-medium text-slate-500 dark:text-slate-400">Light</span>
                </>
              )}
              {themeMode === 'dark' && (
                <>
                  <Moon className="w-4 h-4 text-indigo-400" />
                  <span className="hidden xl:inline text-[11px] font-medium text-slate-500 dark:text-slate-400">Dark</span>
                </>
              )}
            </button>

            {/* 言語切り替えボタン (OS自動 / 日本語 / English) */}
            <button
              onClick={cycleLanguage}
              className="p-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/80 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors flex items-center gap-1.5 text-xs font-semibold"
              title={
                languageMode === 'system'
                  ? dict.nav.langAuto
                  : languageMode === 'ja'
            ...
            >
              ...
              <span className="text-[11px] uppercase tracking-wider">
                {languageMode === 'system' ? `OS (${currentLanguage.toUpperCase()})` : currentLanguage.toUpperCase()}
              </span>
            </button>
          </div>
        </div>
