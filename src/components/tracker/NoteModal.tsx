...
const COMMON_NOTE_TAGS_EN = [
  'Calc Error',
  'Formula Forgotten',
  'Misread Question',
  'Pattern Not Mastered',
  'Time Management',
  'Sign Error (+/-)',
  'Review Definition',
];
...
export const NoteModal: React.FC<NoteModalProps> = ({
  ...
  const { dict, currentLanguage } = useI18n();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-150">
      ...
      >
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-slate-900 dark:text-white">
                  {currentLanguage === 'ja' ? `第 ${question.number} 問のメモ` : `Notes for Q#${question.number}`}
                </h3>
                {/* 現在のステータスバッジ */}
                <span
                ...
                >
                  ...
                  {question.status === 'correct'
                    ...
                    : dict.stats.unansweredLegend}
                </span>
              </div>
              <p className="text-xs text-slate-500">
                {dict.tracker.notePlaceholder}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 space-y-4">
          ...
          {onChangeStatus && (
            <div className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200/70 dark:border-slate-700/60">
              <span className="text-xs font-medium text-slate-600 dark:text-slate-400">
                {currentLanguage === 'ja' ? 'ステータスを変更:' : 'Change Status:'}
              </span>
          ...
          <div>
            <div className="flex items-center gap-1.5 text-xs text-slate-500 mb-2">
              ...
              <span>{currentLanguage === 'ja' ? 'クイック挿入タグ:' : 'Quick Tags:'}</span>
            </div>
          ...
          <div>
            <textarea
              ...
              placeholder={dict.tracker.notePlaceholder}
              rows={6}
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-slate-50/50 dark:bg-slate-800/40 border-t border-slate-100 dark:border-slate-800">
          ...
          >
            {currentLanguage === 'ja' ? 'メモをクリア' : 'Clear Note'}
          </button>
          <div className="flex items-center gap-2">
            <button
              type="button"
            ...
            >
              ...
              <span>{dict.tracker.saveNote}</span>
            </button>
          </div>
        </div>
      </div>
